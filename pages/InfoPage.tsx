...
import { Search, ShieldCheck, Truck, RotateCcw, Lock, Headphones, MapPin, CheckCircle, Send, Loader2 } from 'lucide-react';
...
const InfoPage: React.FC<InfoPageProps> = ({ type }) => {
    const { page: dbPage, loading: pageLoading } = usePage(type as string);
    ...
    const getContent = () => {
        if (pageLoading) {
            return (
                <div className="flex flex-col items-center justify-center p-20 gap-4 text-gray-500">
        ...
        if (dbPage) {
            return (
                <div className="space-y-6">
                    <h1 className="text-3xl font-serif text-gray-900 mb-6">{dbPage.title}</h1>
                    <div
